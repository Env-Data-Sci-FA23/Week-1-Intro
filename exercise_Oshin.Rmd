...
```{r}
environmental_data <- read.csv("data/kenya_county_climates.csv")
```
...
environmental_data %>%
  select(matches("Precipitation") | County) %>%  # Step 1
  filter(Precipitation_585_90 > mean(unlist(select(., -County)), na.rm = TRUE)) %>%  # Step 2
  pivot_longer(cols = -County, names_to = "Variable", values_to = "Value")  # Step 3
...
```{r}
# Calculate the total average for all counties
total_average <- mean(environmental_data$Value, na.rm = TRUE)

# Count the number of counties with predicted precip values greater than the total average
count_counties_above_average <- environmental_data %>%
  filter(Value > total_average) %>%
  distinct(County) %>%
  nrow()

cat("Counties with predicted precip values greater than the total average:", count_counties_above_average, "\n")
...
ggplot(environmental_data, aes(x = Variable, y = Value)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ County, ncol = 3) +
  labs(title = "Precipitation by County",
       x = "Variable",
       y = "Precipitation") +
...
# Extract a vector of unique county names from the data
county_names <- unique(environmental_data$County)

# Create individual bar plots for each county
plots <- map(county_names, function(county) {
  kenya_climates %>%
    filter(County == county) %>%
    ggplot(aes(x = Variable, y = Value)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(title = paste("Precipitation in", county),
         x = "Variable",
         y = "Precipitation") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
})

# Print the individual bar plots
walk(plots, print)
