...
```{r}
kenya_county_climates <- read.csv("/Users/melissamorales/Desktop/Week-1-Intro/data/kenya_county_climates.csv")
```
...
```{r}
# select for County and Precipitation

select(kenya_county_climates, contains("Precipitation"), "County") %>%
filter(Precipitation_585_90> mean(Precipitation_hist)) %>%
...
```{r}
# Mean precip
county_means <- kenya_county_climates %>%
  group_by(County) %>%
  summarize(mean_precipitation = mean(Precipitation_hist))

# Bar plot
ggplot(county_means, aes(x = County, y = mean_precipitation)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  labs(title = "Mean Precipitation by County",
       x = "County",
