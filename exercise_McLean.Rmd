...
```{r}
env <- read_csv("C:/Users/Admin/Desktop/ESS523A/Week-1-Intro/data/kenya_county_climates.csv")
```
...
```{r}
CountyPrecip <- select(env, County, contains("Precipitation"))
```
...
Pivot <- pivot_longer(CountyPrecip, contains("Precipitation"))
...
```{r}
CountyCount <- Pivot %>%
  group_by(County, value)
  summarise(if_else(value > mean(value), "Yes", "No"))
```
...
```{r}
ggplot(Pivot)+
  geom_col(mapping = aes(x = name, y = value))+
