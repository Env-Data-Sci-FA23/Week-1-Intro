...
```{r}
kenya <- read_csv("data/kenya_county_climates.csv")
```
...
```{r}
kenya_precip <- kenya %>%
  select(matches("Precipitation|County"))%>% #select precipitation columns
  filter(Precipitation_585_90 > mean(Precipitation_585_90))%>%
  rename(precip_2030 = Precipitation_585_30,
         precip_2050 = Precipitation_585_50,
         precip_2070 = Precipitation_585_70,
         precip_2090 = Precipitation_585_90,
         precip_hist = Precipitation_hist)

kenya_precip_long <- kenya_precip %>%
  pivot_longer(c(2:6), names_to = "year", values_to = "precip")
```

Question: How many counties have predicted precip values greater than the total average for all counties?

*For 2090, 22 counties have predicted precip values greater greater than the total average for all counties in 2090 (based on the number of rows remaining after filtering the data frame by 2090 precip greater than the mean).*
...
precip_plot <- function(data = kenya_precip_long, county){
  data %>%
    filter(County == "county")%>%
    ggplot()+
    geom_col(aes(x=data$year, y=data$precip)) -> county_plot
  
  county_plot
}
 
#stumped on why my mapping function isn't working...getting an error 'can't convert '.f', a list, to a function'
plot_list <- map(county_names, precip_plot())

#Code checking that function works: 
#precip_plot(data = kenya_precip_long, county = "Baringo") 
#kenya_precip_long %>%
