...
```{r}
#data <- list.files('data/kenya_conty_climate.csv', full.names = TRUE)

Kenya_file <- read_csv('D:/CSU/Rproject/Empty Project/Week-1-Intro/data/kenya_county_climates.csv')
glimpse(Kenya_file)
...
```{r}
Preciptation_data <- select(Kenya_file, County, starts_with('Precipitation'))

Greater_Precip <- filter(Preciptation_data, Precipitation_585_90 > mean(Precipitation_585_90)) 

Greater_Precip %>%  pivot_longer (cols = c('Precipitation_585_30', 'Precipitation_585_50', 'Precipitation_585_70', 'Precipitation_585_90', 'Precipitation_hist' ), names_to=('Precipitation'), values_to ='precip_value')
