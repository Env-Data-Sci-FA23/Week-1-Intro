...
```{r}
library(readr)
kenya_county_climates <- read_csv("~/test-git-intro/Week-1-Intro/data/kenya_county_climates.csv")
...
```{r}
assign_kenya <-kenya_county_climates %>%
  select(matches("Precipitation|County"))%>%
...
Question: How many counties have predicted precip values greater than the total average for all counties? ##\*n_distinct(assign_kenya\$County) = 22 distinct counties greater than the mean precip\*
...
```{r}
ggplot(assign_kenya, aes(fill = County)) +
  geom_col(x = County, y = value)
