...
```{r}
Kenya <- read.csv("data/kenya_county_climates.csv")
```
...
KenyaPre <- Kenya %>%
select(County,contains("Precipitation")) %>%
filter(Precipitation_585_90 > mean(Precipitation_585_90,na.rm= F))
```
...
```{r}
  Kenyapivot <- pivot_longer(KenyaPre,contains("Precipitation"))
