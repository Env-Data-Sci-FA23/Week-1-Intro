...
```{r}
kdata <- read.csv("data/kenya_county_climates.csv")
view(kdata)
```
...
```{r}
kdata2 <- kdata %>% select(contains("Precipitation") | contains ("County")) %>%
  filter(Precipitation_585_90 > mean(Precipitation_585_90)) %>% 
  pivot_longer(cols = contains("Precipitation"))

view(kdata2) 

#How many counties have predicted precip values greater than the total average for all counties?

Precipitation <- kdata2$value

mean_precip <- kdata2 %>% summarise(avg = mean(!!sym(Precipitation))) %>% pull(avg)
...
ggplot(kdata2, aes(x = name, y = value, fill = County)) +
  geom_col()+ 
  facet_wrap(~ County)
